...
public class BTreeFile implements DbFile {
	...
	private Iterator<BTreeEntry> bit;
	private Iterator<BTreeEntry> internal;
	...
	private BTreeLeafPage findLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
		...
		if (f == null)
		{
			...
			{	
				left = bit.next();					
				return findLeafPage(tid, left.getLeftChild(), Permissions.READ_ONLY, f);
			}
		}
		else
		{	
			if (bit.hasNext())
			{
				BTreeEntry first = bit.next();
				Boolean less = f.compare(Op.LESS_THAN, first.getKey());
				if (less)
					return findLeafPage(tid, first.getLeftChild(), Permissions.READ_ONLY, f);
				else
				{
					while (bit.hasNext() && f.compare(Op.GREATER_THAN, first.getKey()))
						first = bit.next();
					Boolean lessthan = f.compare(Op.LESS_THAN_OR_EQ, first.getKey());
					if (lessthan)
						return findLeafPage(tid, first.getLeftChild(), Permissions.READ_ONLY, f);	
					else
						return findLeafPage(tid, first.getRightChild(), Permissions.READ_ONLY, f);	
	...
	protected BTreeLeafPage splitLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage page, Field field) 
		...
		BTreeLeafPage newLeft = (BTreeLeafPage) getPage(tid, dirtypages, page.getId(), Permissions.READ_WRITE);
		BTreePageId rightID = (BTreePageId) newRight.getId();
		BTreePageId leftID = (BTreePageId) newLeft.getId();
	
		int middle = (int) Math.ceil(page.getNumTuples() / 2.0); //getting the middle tuple
	
		Iterator<Tuple> leaf = newLeft.reverseIterator();
		Field middleField;
		Tuple add = null;
		System.out.println(page.getTuple(page.getNumTuples()-1));
		
		for (int i = page.getNumTuples() - 1; i >= middle; i--)
		{
			if (leaf.hasNext())
			{
				add = leaf.next();
				newLeft.deleteTuple(add);
				newRight.insertTuple(add);
			}
			
		}

		middleField = add.getField(keyField);
		BTreeEntry newEntry = new BTreeEntry(middleField, leftID, rightID);
		
		newRight.setLeftSiblingId(leftID);
		newRight.setRightSiblingId(newLeft.getRightSiblingId());
		newLeft.setRightSiblingId(rightID);
		
		BTreeInternalPage newParent = getParentWithEmptySlots(tid, dirtypages, page.getParentId(), field);
		newRight.setParentId(newParent.getParentId());
		newLeft.setParentId(newParent.getParentId());
		
		newParent.insertEntry(newEntry);
		newParent.updateEntry(newEntry);
		updateParentPointers(tid, dirtypages, newParent);
		
		if (field.compare(Op.GREATER_THAN_OR_EQ, middleField))
			return newRight;
		else
			return newLeft;
	...
	protected BTreeInternalPage splitInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		BTreeInternalPage newLeft = (BTreeInternalPage) getPage(tid, dirtypages, page.getId(), Permissions.READ_WRITE);
		BTreePageId rightID = (BTreePageId) newRight.getId();
		BTreePageId leftID = (BTreePageId) newLeft.getId();
		
		int middle = (int) Math.ceil(page.getNumEntries() / 2.0);
		Field middleField;
		BTreeEntry add = null;
		
		Iterator<BTreeEntry> internal = newLeft.reverseIterator();
		
		for (int i = page.getNumEntries(); i > middle; i--)
		{
			if (internal.hasNext())
			{
				add = internal.next();
				newLeft.deleteKeyAndRightChild(add);
				newRight.insertEntry(add);
			}
		}
		add = internal.next();
		middleField = add.getKey();
	    ...
	    //updateParentPointer(tid, dirtypages, leftID, rightID);
	    
	    if (field.compare(Op.GREATER_THAN, middleField))
			return newRight;
		else
			return newLeft;
	}
	...
	protected void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
		...
		int leafNodeLength = page.getNumTuples();
		int siblingLength = sibling.getNumTuples();
		
		Iterator<Tuple> leaf;
		Tuple leafTuple = null;
		
		if (!isRightSibling)
			leaf = sibling.reverseIterator();
		else
			leaf = sibling.iterator();
		
		while (leafNodeLength < siblingLength)
		{
			if (leaf.hasNext())
			{
				leafTuple = leaf.next();
				sibling.deleteTuple(leafTuple);
				page.insertTuple(leafTuple);
			}
		...
		Field newKey = leafTuple.getField(keyField);
		entry.setKey(newKey);
	...
	protected void stealFromLeftInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		BTreeInternalPage left = (BTreeInternalPage) getPage(tid, dirtypages, leftSibling.getId(), Permissions.READ_WRITE);
		BTreeInternalPage right = (BTreeInternalPage) getPage(tid, dirtypages, page.getId(), Permissions.READ_WRITE);
		BTreeInternalPage parentPage = (BTreeInternalPage) getPage(tid, dirtypages, parent.getId(), Permissions.READ_WRITE);
		
		int internalNodeLength = right.getNumEntries();
		int leftSibLength = left.getNumEntries();
		
		Iterator<BTreeEntry> internal = left.reverseIterator();
		
		BTreeEntry swappedEntry = null;
		BTreeEntry newEntry = null;
		
		while (internalNodeLength < leftSibLength)
		{
			if (internal.hasNext())
			{
				swappedEntry = internal.next();
				
				left.deleteKeyAndRightChild(swappedEntry);
				//right.updateEntry(leftEntry);
				newEntry = new BTreeEntry(leftEntry.getKey(), swappedEntry.getRightChild(), right.iterator().next().getLeftChild()); //rotate down the parent key to be the new right child first key
			
				right.insertEntry(newEntry);
				leftEntry.setKey(swappedEntry.getKey()); //rotate up the left sib key to the parent key
		...
		updateParentPointer(tid, dirtypages, left.getId(), right.getId());
	...
	protected void stealFromRightInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		// the corresponding parent entry. Be sure to update the parent
		// pointers of all children in the entries that were moved.
		BTreeInternalPage left = (BTreeInternalPage) getPage(tid, dirtypages, page.getId(), Permissions.READ_WRITE);
		BTreeInternalPage right = (BTreeInternalPage) getPage(tid, dirtypages, rightSibling.getId(), Permissions.READ_WRITE);
		BTreeInternalPage parentPage = (BTreeInternalPage) getPage(tid, dirtypages, parent.getId(), Permissions.READ_WRITE);
		
		int internalNodeLength = left.getNumEntries();
		int leftSibLength = right.getNumEntries();
		
		Iterator<BTreeEntry> internal = right.iterator();
		...
		while (internalNodeLength < leftSibLength)
		{
			...
			{
				swappedEntry = internal.next();
				right.deleteKeyAndLeftChild(swappedEntry);
				newEntry = new BTreeEntry(rightEntry.getKey(), left.reverseIterator().next().getRightChild(),swappedEntry.getLeftChild()); //rotate down the parent key to be the new right child first ke			
				left.insertEntry(newEntry);
				rightEntry.setKey(swappedEntry.getKey()); //rotate up the left sib key to the parent key
		...
		//updateParentPointer(tid, dirtypages, left.getId(), right.getId());
		updateParentPointers(tid, dirtypages, left);
	}
	...
	protected void mergeLeafPages(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		// the sibling pointers, and make the right page available for reuse using setEmptyPage.
		// Delete the entry in the parent corresponding to the two pages that are merging -
		// deleteParentEntry() will be useful here
		
		BTreeLeafPage right = (BTreeLeafPage) getPage(tid, dirtypages, rightPage.getId(), Permissions.READ_WRITE);
		BTreeLeafPage left = (BTreeLeafPage) getPage(tid, dirtypages, leftPage.getId(), Permissions.READ_WRITE);
		BTreeInternalPage parentPage = (BTreeInternalPage) getPage(tid, dirtypages, parent.getId(), Permissions.READ_WRITE);
		
		int rightLength = right.getNumTuples();
		Tuple newTuple = null;
		
		Iterator<Tuple> leaf = right.iterator();
		...
		for (int i = 0; i < rightLength; i++)
		{
			...
			{
				newTuple = leaf.next();
				right.deleteTuple(newTuple);
				left.insertTuple(newTuple);
		...
		deleteParentEntry(tid, dirtypages, left, parentPage, parentEntry);
		setEmptyPage(tid, dirtypages, right.getId().pageNumber());
	...
	protected void mergeInternalPages(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		//read-write permissions for bufferpool to work properly
		BTreeInternalPage right = (BTreeInternalPage) getPage(tid, dirtypages, rightPage.getId(), Permissions.READ_WRITE); 
		BTreeInternalPage left = (BTreeInternalPage) getPage(tid, dirtypages, leftPage.getId(), Permissions.READ_WRITE);
		BTreeInternalPage parentPage = (BTreeInternalPage) getPage(tid, dirtypages, parent.getId(), Permissions.READ_WRITE);
		
		
		int rightLength = right.getNumEntries();
		BTreeEntry swapped = null;
		
		Iterator<BTreeEntry> internal = right.iterator();
		
		BTreeEntry newEntry = new BTreeEntry(parentEntry.getKey(), left.reverseIterator().next().getRightChild(), right.iterator().next().getLeftChild());
		left.insertEntry(newEntry);
		...
		for (int i = 0; i <= rightLength + 1; i++)
		{
			if (internal.hasNext())
			{
				swapped = internal.next();
				right.deleteKeyAndLeftChild(swapped);
				left.insertEntry(swapped);
				
			}
		}
		
		deleteParentEntry(tid, dirtypages, left, parent, parentEntry);
		setEmptyPage(tid, dirtypages, right.getId().pageNumber());
